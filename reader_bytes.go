...
type ReaderBytesOptions struct {
...
func NewReaderBytes() ReaderBytes {
	r := ReaderBytes{
		reassembled: make(chan []Bytes),
		done:        make(chan bool),
		first:       true,
		initiated:   true,
	}
...
func (r *ReaderBytes) Reassembled(reassembly []Bytes) {
	if !r.initiated {
		panic("ReaderBytes not created via NewReaderStream")
	}
...
func (r *ReaderBytes) ReassemblyComplete() {
	close(r.reassembled)
	close(r.done)
}
...
// slices.
func (r *ReaderBytes) stripEmpty() error {
...
func (r *ReaderBytes) Read(p []byte) (int, error) {
	...
	if !r.initiated {
		panic("ReaderBytes not created via NewReaderBytes")
	}
	...
	if len(r.current) > 0 {
		current := &r.current[0]
		if r.LossErrors && !r.lossReported {
			r.lossReported = true
			return 0, DataLost
		}
...
func (r *ReaderBytes) Close() error {
